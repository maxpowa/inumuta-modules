...
def f_time(bot, trigger):
    ...
    if trigger.group(2):
        zone = get_timezone(bot.db, bot.config, trigger.group(2).strip().lower(), None, None)
        if not zone:
        ...
        zone = get_timezone(bot.db, bot.config, None, trigger.nick.lower(),
                            trigger.sender)
...
def update_user(bot, trigger):
    ...
    if not pytz:
        ...
        bot.db.preferences.update(trigger.nick.lower(), {'tz': tz})
        if len(tz) < 7:
