...
Emoji list adapted from https://github.com/carpedm20/emoji/blob/master/emoji/code.py (MIT)
...
def show_rule(bot, trigger):
    """
    .emoji <id> - Attempt to show the requested emoji (Some systems will not support the character)
    """
    if not trigger.group(2):
        return
        
    emoji = trigger.group(2).strip().lower() 
    
    if emoji in allEmoji:
        bot.say(u'Emoji {0}: {1}'.format(emoji,allEmoji[emoji]))
    else:
        bot.say(u'Sorry, the emoji \'{}\' does not exist.'.format(emoji))
